...
let groupScope = ''
/** Names the tab being built so the same group title in two tabs folds independently. */
export function setGroupScope(name: string): void {
  groupScope = name
}
...
export function group(title: string, ...kids: Child[]): HTMLElement {
  const key = groupScope + '/' + title
  const body = h('div', { class: 'group-body' }, kids)
  const head = h('button', { type: 'button', class: 'group-head', 'aria-expanded': String(!collapsed.has(key)) }, icon('chevron', 14), h('span', null, title))
  const sec = h('section', { class: 'group' + (collapsed.has(key) ? ' folded' : '') }, head, body)
  head.addEventListener('click', () => {
    const fold = !sec.classList.contains('folded')
    sec.classList.toggle('folded', fold)
    head.setAttribute('aria-expanded', String(!fold))
    if (fold) collapsed.add(key)
